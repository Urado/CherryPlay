...
- **Рекомендуемый путь:** `GET /health` или `GET /api/health`.
- **Ожидаемое поведение:** при работоспособном сервере — ответ 200, при необходимости — минимальный JSON (например, `{ "status": "Healthy" }`).

Детали реализации (путь, формат ответа) задаются при реализации Epic G.
