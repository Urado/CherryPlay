...
*Связь с учётной записью (OAuth провайдеры: Telegram, VK, Mail.ru) хранится в таблице `OAuthAccounts` (см. ниже). В v1 один организатор может иметь несколько привязок к разным провайдерам (например, войти через Telegram, затем привязать VK).*
...
| `Provider` | string | NOT NULL, CHECK IN ('telegram', 'vk', 'mailru') | Провайдер OAuth: `telegram`, `vk` или `mailru`. |
...
3. Если организатор уже авторизован, можно добавить дополнительную привязку к другому провайдеру (например, войти через Telegram, затем привязать VK для удобства).
