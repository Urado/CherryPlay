...
Use this skill to collect enough clarity through guided discovery, then run the large-task orchestration only after explicit user confirmation.
...
- User asks for immediate implementation without a discovery phase.
- Task is small enough for direct worker execution.
...
- **Hard cap:** ask at most **3 rounds** of agent clarifying questions total. If uncertainty remains after round 3, switch to explicit assumptions, list residual risks, and wait for the user command to start implementation.
...
4. **Ask additional questions (if needed)**  
   ...
   - Apply the hard cap: once 3 question rounds are reached, do not ask more questions; move to explicit assumptions and residual risks.
...
- **Stage 0** — Already done: `{prefix}-00-chat-summary.md` was written in step 1 above (temporary; kept by default, removable manually later). (If orchestration is started without discovery, the orchestrator chooses the prefix and performs Stage 0 inside large-task-orchestration.)
- **Stage 1** — **Business-analyst** reads `{prefix}-00-chat-summary.md` and writes the technical spec to `{prefix}-01-technical-spec.md` (temporary; kept by default, removable manually later).
...
| Phase | What happens                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | Discovery: analyst lens → restatement, assumptions, risks. Questions to the user — **single continuous numbering (1, 2, 3, …)**. After each answer: analyze whether there are enough details; if not — more questions **(continue numbering: 5, 6, 7…)**, with a hard cap of **3 rounds**. After cap: proceed with explicit assumptions and residual risks. Then: "Write 'let's go' / 'run the workflow' to start." |
...
- During Phase 1, stop after questions/clarifications and wait for user command.
- During Phase 2, stop after orchestration summary and open follow-ups.
