...
- **deploy.sh** собирает `.env` из `VERSION`, `REGISTRY`, `IMAGE_`_, затем дополняет из `.env.production`, если файл есть, затем переопределяет секретами из CI (JWT*SECRET_KEY, POSTGRES_PASSWORD, CORS_ORIGIN*_ и т.д.). См. [scripts/deploy.sh](scripts/deploy.sh).
