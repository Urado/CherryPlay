...
Опубликованные GitHub Release builds используют `dist:win:ci` и **пока без** нативного AIMP bridge (отложено). PR в `main`/`develop` с изменениями в List/Components запускают **Verify Desktop Windows Zip** (`verify-desktop-windows.yml`) — тот же `dist:win:ci`; zip можно скачать из run (**Artifacts** → `cherryplaylist-win-x64`) или по ссылке из комментария бота в PR — для теста до релиза. Локальный `dist:win` — для мейнтейнеров с собранным плагином.
