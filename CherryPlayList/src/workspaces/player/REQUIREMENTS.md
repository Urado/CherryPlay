...
1. **"Пауза в конце трека" (pause)**
   - Остаёмся на **текущем** закончившемся треке; **автоперехода** к следующему нет
   - Чтобы продолжить очередь, пользователь нажимает **«Следующий»** (Next)
   - Play на том же треке (если применимо) — по обычным правилам transport; режим не подгружает следующий трек сам

2. **"Пауза между треками" / «Интервал» (pauseAndNext)**
   - Ждём время, заданное пользователем (настройка интервала между треками)
   ...
   - Интервал учитывается в расчёте времени окончания для каждого трека с таким действием (как дополнительное время к длительности трека)

3. **"Сплошное воспроизведение" / «Без паузы» (next)**
...
- Если выбрано "сплошное воспроизведение" / «Без паузы» (next), сразу переходим к следующему треку без задержки
- Если выбрано "пауза между треками" / «Интервал» (pauseAndNext), происходит пауза на заданное время, затем автоматически запускается следующий трек
- Если выбрано **"пауза в конце трека"** (pause), **автоперехода нет** — остаёмся на текущем закончившемся треке; переход вручную кнопкой **«Следующий»**
...
- Демо-плеер — общая система предпрослушивания: **плавающая панель** и/или layout-зона `demo-player` (не в AppHeader). Основной плеер сессии размещается только в зоне `player`, **не** в шапке приложения
- Кнопка предпрослушивания (Play у трека в списке) **всегда доступна** — и в режиме подготовки, и в режиме сессии (не скрывается из‑за session)
- Если воспроизведение demo **заблокировано** (например, то же аудиоустройство, что у основного плеера в session): UI демо-плеера **всё равно открывается**, звук не идёт, показывается явное сообщение **«Воспроизведение невозможно»** (или эквивалент с пояснением про устройство) — silent no-op запрещён
- Если плеер выберет устройство, на котором уже играет демо-плеер:
  ...
  - Демо-плеер становится неактивным по звуку (`isDisabled`); UI и кнопка предпрослушивания остаются доступными по политике выше
...
- Если демо-плеер выбран на то же устройство, что и плеер, но демо-плеер на паузе:
  - В режиме сессии демо-плеер блокируется по звуку так же (`isDisabled`)
...
1. **Информационная панель (верхняя часть)**
   ...
   - Кнопка предпрослушивания у трека в списке **всегда** доступна (preparation и session); при блокировке — открытие UI демо-плеера + «Воспроизведение невозможно» (см. §6.2)
...
- Если у трека или группы настроено нестандартное поведение перехода к следующему треку (отличающееся от настроек по умолчанию), рядом с иконкой настроек появляется одна из трёх иконок:
  - Иконка "пауза в конце трека" (⏸) — если настроено действие "pause"
  - Иконка "пауза между треками" / интервал (⏸⏭) — если настроено действие "pauseAndNext"
...
Добавлен новый layout preset `'player'`, который включает:
...
- Использует `projectStore` для данных плейлиста (треки, группы, настройки)
- Использует `playerAudioStore` для управления аудио воспроизведением
- Использует `playerSessionStore` для состояния сессии (режим, проигранные треки)
- Модуль не должен зависеть от других workspace, но использует общий `projectStore`
- Сохраняет состояние через persist middleware (projectStore, playerSessionStore)
