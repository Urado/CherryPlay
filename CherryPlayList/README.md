...
- Перетаскивание треков между подборками и плейлистами
...
- UI зоны: **«Подборка»**; layout presets **«Сборка плейлиста»** (`collections`, `collections-vertical`)

### 3. Предпросмотр (только у вас)

- Плавающая панель предпрослушивания (и опциональная layout-зона `demo-player`); UI title **«Предпросмотр (только у вас)»**; основной плеер — в зоне **Проигрывание**, не в шапке
...
- Кнопка «Показать в файлах» переводит зону **Файлы** (`fileBrowser`) к исходному файлу
...
1. **Файлы** (`fileBrowser`) - просмотр и выбор треков из локальной библиотеки
2. **Редактор плейлиста** - область для формирования последовательности треков
3. **Подборки** - облегчённые плейлисты для временного хранения треков (без ограничений; UI **«Подборка»**)
4. **Система перетаскивания** - универсальный workspace-agnostic механизм drag & drop:
   ...
   - Перетаскивание внутри workspace (плейлист, подборка, будущие типы)
   - Cross-workspace операции (между любыми workspace: плейлист ↔ подборка, подборка ↔ подборка и т.д.)
   - Перетаскивание из панели **Файлы**
...
5. **Предпросмотр (только у вас)** - воспроизведение выбранного трека без очереди, отображение состояния и управление звуком
...
7. **Кастомизация интерфейса** - настройки внешнего вида приложения:
   ...
   - Унифицированные размеры строк в плейлисте и зоне **Файлы**
...
- ✅ **Подборки** - облегчённые плейлисты для временного хранения (без ограничений; UI **«Подборка»**)
...
- ✅ **Предпросмотр (только у вас)** - воспроизведение треков с кнопками в плейлистах и подборках
  - Кнопки воспроизведения в плейлистах и коллекциях
  - Плеер без очереди: в конце трек останавливается
  - Кнопка «Показать в файлах» и индикаторы времени/таймлайн
