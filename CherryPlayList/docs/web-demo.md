...
| `VITE_API_URL`           | опционально                    | Базовый URL CherryPlayServer в демо (по умолчанию `http://localhost:5000` в `demoConfig`) |

Пример с явным URL сервера:
...
- **Сброс persist** при старте демо — чтобы не подтягивать токен/пути из прошлой сессии Electron в том же origin (IndexedDB).
...
| **Вечеринка / Party**      | UI на фикстурах `demoPartyFixture` (вечеринка `DEMODK`); HTTP/SignalR к серверу нет                                                                        |
| **Аккаунт**                | Фейковый организатор «Demo Organizer»; login/OAuth без запросов к API                                                                                      |
...
Вне scope (не обещаются в этом режиме): File System Access API, Android WebView, полноценный CherryPlayServer/SignalR в браузере, CherryPlayWeb, переименование `electronStorage`, mp3 в репозитории.
