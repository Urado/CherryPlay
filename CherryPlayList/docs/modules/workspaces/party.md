# Party

Workspace для создания и управления вечеринками с трансляцией плейлиста.

## Описание

Модуль позволяет организаторам создавать вечеринки с выбором стиля оформления, настраивать кастомизацию и получать уникальный URL для веб-страницы вечеринки.

## Основные компоненты

- **PartyView** (`src/workspaces/party/PartyView.tsx`) - Основной компонент workspace
- **PartyTrackDisplaySection** (`src/workspaces/party/components/PartyTrackDisplaySection.tsx`) — блок **«Отображение треков»** (настройки имён треков для превью и публикации; см. ниже)
- **PartyEditor** (`src/workspaces/party/components/PartyEditor.tsx`) — Форма создания и редактирования вечеринки (название, тема, кастомизация; поля карточки каталога: краткое описание, город, дата/время **начала** и опционального **конца** мероприятия, внешняя ссылка, теги танцев). Данные сохраняются через API (CreatePartyDto/UpdatePartyDto) в сущность Party на сервере, в том числе поля `eventDateTime` и `eventEndDateTime`.
- **PartyPreview** (`src/workspaces/party/PartyPreview.tsx`) - Превью плейлиста
- **PartyView.css** (`src/workspaces/party/PartyView.css`) — стили компонента, включая экран недоступности сервера (`.party-view-no-connection`)
- **projectStore** (`src/shared/stores/projectStore.ts`) — хранит `meta.linkedParty` (`{ id, shortCode }`); `url` не персистируется и регенерируется при загрузке через `partyService.getPartyUrl`
- **partyService** (`src/shared/services/partyService.ts`) - Сервис для работы с API

## Отображение имён треков (party track display)

Настройки того, **как показываются имена треков** в веб‑превью плейлиста в Party workspace и в **плейлисте, который уходит на сервер** (публикация, SignalR, модальное окно привязки вечеринки, AIMP‑интеграция). При включении опции с начала имени снимается заданное число **Unicode code points** (см. тип `PartyTrackDisplaySettings` в [`project.ts`](../../../src/core/types/project.ts): `stripLeadingCharsEnabled`, `stripLeadingCharsCount`).

- **Не** относится к JSON кастомизации темы на сервере (`customizationSettings` Party API) — это отдельные **локальные/проектные** поля.
- **Хранение:** `meta.partyTrackDisplay` в [`projectStore`](../../../src/shared/stores/projectStore.ts): опциональный ключ `partyTrackDisplay` в файле проекта `.cherry` и персист Zustand для метаданных проекта.
- **UI:** секция рендерится в [`PartyView.tsx`](../../../src/workspaces/party/PartyView.tsx) **над** `PartyEditor`, заголовок блока — «Отображение треков».
- **Преобразование имён:** логика в [`partyUtils.ts`](../../../src/shared/utils/partyUtils.ts) (`applyPartyTrackDisplayToTrackName`, `applyPartyTrackDisplayToComponentPlaylist`, `convertPlaylistForApi` / `convertAimpPlaylistForApi`). Исходные имена треков в проекте **не меняются** — изменяются только данные для превью и для API‑плейлиста.

## Функциональность

- Создание вечеринки с названием и выбором стиля (Cyberpunk, Sakura, Art Deco, Базовый, Spring Cross Step)
- Настройка кастомизации для выбранного стиля
- Для стиля `basic`: выбор палитры (`base` + 16 предустановленных + `custom`) и редактирование 5 цветов в режиме `custom` (color picker + HEX)
- Для стиля `basic`: `custom` отображается вторым пунктом в списке палитр (сразу после `base`)
- Для стиля `basic`: при выборе предустановленной палитры её цвета сохраняются в `customPalette`; при переключении на `custom` подставляются цвета последней выбранной палитры
- Превью плейлиста с применением стиля
- Получение уникального URL для веб-страницы
- Сохранение состояния вечеринки между сессиями
- Обработка потери соединения с сервером с автоматическим переподключением
- Интеграция с SignalR для трансляции (в разработке)

Для `basic` канонический формат `customizationSettings` в запросах Party API: `{ paletteId, customPalette }`, где `customPalette` содержит 5 цветов (`accentPrimary`, `textPrimary`, `backgroundPrimary`, `trackAreaBackground`, `trackBackground`). Палитра по умолчанию — `base`. Legacy-flat ключи `custom*` остаются совместимыми при чтении существующих данных.

Сервер принимает `customizationSettings` как generic JSON и не применяет строгую per-theme валидацию схемы.

### Дата и время мероприятия

В `PartyEditor` блок даты/времени позволяет:

- задать время **начала** вечеринки (`eventDateTime`) в выбранной таймзоне организатора;
- при наличии начала — задать опциональное время **окончания** (`eventEndDateTime`), чтобы веб‑каталог мог показывать интервал **«с HH:MM до HH:MM»**;
- в любой момент изменить начало и/или конец (новые значения отправляются на сервер через `UpdatePartyDto`);
- очистить конец (установить «без конца»): при этом в запросе на сервер `eventEndDateTime` становится `null`/опускается, а в публичных DTO поле отсутствует, и CherryPlayWeb показывает только начало или полностью скрывает строку интервала.

Все изменения времени синхронизируются с сервером и попадают в публичные DTO (`PublicPartyDto`, `PublicPartyListItemDto`), которые использует CherryPlayWeb для отрисовки карточек каталога (см. [CONTRACTS.md](../../../../CONTRACTS.md) §6.4).

## Обработка потери соединения

При наличии привязанной вечеринки (`linkedParty`) `PartyView` проверяет доступность сервера при каждом запуске приложения.

### Проверка доступности

`partyService.checkServerReachable()` — HEAD-запрос к `/api/parties` с таймаутом 5 секунд. Возвращает `true`, если сервер ответил со статусом < 500.

### Поведение при недоступном сервере

- Весь блок Party workspace заменяется экраном «Не удалось подключиться к серверу».
- Запускается таймер с интервалом 60 секунд, повторяющий проверку автоматически.
- Кнопка **«Проверить сейчас»** инициирует немедленную проверку вне зависимости от таймера.

### Восстановление соединения

При успешной проверке:

1. Перезагружаются URL и метаданные вечеринки.
2. Таймер переподключения останавливается.
3. Отображается штатный интерфейс Party workspace.

### Состояния в PartyView

| Состояние               | Тип       | Назначение                                   |
| ----------------------- | --------- | -------------------------------------------- |
| `serverUnreachable`     | `boolean` | Флаг недоступности сервера                   |
| `isReconnecting`        | `boolean` | Флаг активной проверки соединения            |
| `reconnectIntervalRef`  | `ref`     | Ссылка на интервал переподключения           |
| `reconnectCancelledRef` | `ref`     | Защита от race condition при размонтировании |

При размонтировании компонента таймер очищается автоматически.

## Зависимости

- `@cherryplay/components` - библиотека компонентов для отображения плейлиста
- `projectStore` - данные плейлиста для трансляции (projectStore заменяет устаревший playerItemsStore)

## Связь со Streaming System и Player

Party workspace отвечает за **жизненный цикл вечеринки**:

- создание записи на сервере (через `partyService`);
- получение `partyId`, `shortCode` и публичного URL;
- сохранение `{ id, shortCode }` в `projectStore.meta.linkedParty`; `url` не сохраняется и регенерируется при каждом запуске приложения или загрузке файла.

Дальше в работу вступают другие модули:

- **Streaming System** (`../systems/streaming.md`):
  - использует `partyId` и `shortCode` для подключения к SignalR Hub;
  - синхронизирует плейлист и состояние воспроизведения с сервером;
  - обеспечивает доставку состояния до CherryPlayWeb.

- **Player workspace** (`./player.md`):
  - при наличии созданной вечеринки и активного подключения к серверу:
    - транслирует позицию и состояние воспроизведения;
    - обновляет плейлист вечеринки на сервере;
    - служит источником правды для Streaming System.

В итоге связка модулей выглядит так:

1. **Party** — создаёт вечеринку и выдаёт URL.
2. **Player** — запускает сессию воспроизведения.
3. **Streaming** — транслирует состояние для зрителей.

Подробнее о протоколах, потоках данных и серверной части см. модуль [Streaming](../systems/streaming.md) и [docs/integration/streaming.md](../../../../docs/integration/streaming.md).
