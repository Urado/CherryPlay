...
- **`partyProgramEndedStore`** — эфемерный `programEnded` + reminder на пульте; **не** персистируется; сброс при fresh project и runtime-условиях (см. матрицу сбросов / [§7.6](../../party-header-control-ux.md#76-конец-программы--доиграл-последний-трек)).
...
| **Пульт**         | AppHeader                                                         | CTA матрица (§7); Publish ↑ + ⚙ только при `linkedParty` (`publishPartyToSite` / модал настроек); Unarchive с confirm (только здесь для привязанного проекта)                                                                                                                                                                                                                                                                                                                                |
