...
> **Термины в UI и в коде:** в интерфейсе — **«Начать проигрывание»** / **«Остановить проигрывание»**; в коде и persist — `session` / `sessionState.mode`. Синхронизация состояния с сервером для гостей — настройка **«Онлайн»** (`enableStreaming`). См. [GLOSSARY](../../../../GLOSSARY.md) (термин **session**, таблица UI).
