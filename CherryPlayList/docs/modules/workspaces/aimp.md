...
- Пресет **«Играть для гостей»** (`party`) — player + настройка + превью; источник CherryPlay или AIMP переключается в зоне Проигрывание.
- **`aimp-party`** — legacy; не показывается в меню, persist мигрирует на `party`.

## Публикация и троттлинг

- **Источник проигрывания** — **Настройки → Источник проигрывания** (CherryPlay / AIMP); среди playback-related настроек — первым.
