...
| 2026-08-07 | **Party create → `ready`, no return-to-draft:** `POST /api/parties` сразу `ready` (**Ждёт начала**); UI без **«Вернуть в черновик»** / **«Сделать черновиком»**; `ready` → `draft` запрещён (**409**); **Опубликовать** только для legacy `draft`; create default **«По ссылке»** (`isListedInCatalog: false`, optional `true`). См. [GLOSSARY — lifecycle actions](../../GLOSSARY.md#cherryplaylist-lifecycle-и-действия-party-editor), [party.md](./modules/workspaces/party.md), [CONTRACTS.md](../../CONTRACTS.md) |)
