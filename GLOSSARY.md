...
| **Publish**              | **Код/API:** PUT плейлиста и метаданных (`updatePartyPlaylist`, `updateParty`). **UI:** **«Обновить на сайте»** / **«Обновить для гостей»**. **Не путать** с lifecycle **«Сделать доступной»** (`draft` → `ready`, только для оставшихся черновиков) — тот меняет статус страницы без обязательного обновления плейлиста. Новая вечеринка после **Создать** сразу в `ready` (**Ждёт начала**); шаг **Сделать доступной** не обязателен. |
...
| **catalog** (в каталоге) | Вечеринка в общем каталоге для зрителей. **UI:** **«В каталоге»** (`isListedInCatalog=true`). Управление флагом в Party Editor / кабинете / **Мои вечеринки** — при lifecycle `ready` и `completed` (чтобы архивная вечеринка не зависала listed без UI). Публичный каталог может включать `completed`, если listed. `draft` в публичный каталог не входит. (`layout="header"`: кнопка без текстовой подсказки; строка «Отдельно от статуса…» — только в `layout="default"`, в shipped Editor не используется). |
...
| Party Editor shell phase badge                      | Обычно **скрыт**, когда показаны lifecycle-controls (`hidePhaseBadge={showLifecycle}`: фазы `draft-linked` / `ready` / `completed` с **Вернуть из архива**). Badge **В архиве** — когда controls не закрывают статус |
...
После **Создать** (`POST /api/parties`) вечеринка сразу в lifecycle `ready` (**Ждёт начала**). Каталог (**По ссылке** / **В каталоге**) — отдельно от lifecycle и от Publish (**Обновить на сайте** / **Обновить для гостей**): Publish ≠ каталог. Возврат `ready` → `draft` снят с продукта (API → **409** `invalid_lifecycle_transition`). Архив (`completed`) **не** терминальный: можно вернуть в `ready`.

| Код / действие                       | Русский UI                                                                                                                                 |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `transitionPartyLifecycle` → `ready` | из `draft`: **Сделать доступной** (legacy / `draft-linked`; happy-path create этот шаг не требует); из `completed`: **Вернуть из архива** |
| → `completed`                        | **В архив**                                                                                                                                |
| `completed` → `ready`                | **Вернуть из архива** (разрешён API и UI)                                                                                                  |
| `ready` → `draft`                    | В UI **нет**; сервер **запрещает** (**409** `invalid_lifecycle_transition`)                                                                |
| Publish (плейлист + метаданные)      | **Обновить для гостей** / **Обновить на сайте** — не путать с **Сделать доступной** (lifecycle) и с toggle каталога                        |
| **Скопировать URL**                  | Видна при `draft-linked`, `ready` и `completed` (есть `linkedParty.url`)                                                                   |
| Успех lifecycle                      | Клиент **может** показать toast (напр. «Вечеринка в архиве»); **CherryPlayList** success-toast для lifecycle **не** показывает — обновляются badge/кнопки; Web при ошибке — сообщение, success-toast не обязателен |
...
| `ready`          | **Настройки вечеринки**      | То же: **Ждёт начала** / **Идёт** не показываются в shell, пока видны lifecycle-controls               |
| `completed`      | **Вечеринка в архиве**       | При видимом **Вернуть из архива** badge может быть скрыт (`hidePhaseBadge`); иначе **В архиве**  |

Первичный статус организатора при **Онлайн** — [AppHeader](#cherryplaylist-header-party-status), не shell badge. Нумерованной ready-phase подсказки в Editor **нет**. Контроль каталога (**«По ссылке»** / **«В каталоге»**) — в фазах `ready` и `completed`.
