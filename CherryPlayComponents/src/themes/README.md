...
Светлая весенняя тема с зелёными акцентами. Тема переопределяет **CurrentTrackDisplay**: помимо блока «Сейчас играет» показываются опциональные строки «Предыдущий» (выше) и «Следующий» (ниже). Порядок — плоский плейлист (только треки, по display order; группы игнорируются). Предыдущий/следующий отображаются только при наличии и при более чем одном треке в плейлисте. Строки стилизованы менее заметно (меньше, прозрачнее); текущий блок остаётся в фокусе.
