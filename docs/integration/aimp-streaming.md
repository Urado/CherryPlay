...
- **environment eligible** — ОС Windows, архитектура x64, найден манифест плагина (`plugins/CherryPlayAimpBridge/manifest.json`), источник = AIMP. Без этого выбор AIMP в настройках недоступен или пресет «AIMP + Party» скрыт.
...
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Источник AIMP недоступен в настройках**      | Windows x64; наличие `plugins/CherryPlayAimpBridge/manifest.json` (в dev — рядом с приложением, в установленном — в `resources/plugins/CherryPlayAimpBridge/` или рядом с exe в `CherryPlayAimpBridge/`); при необходимости задать путь к манифесту/ресурсам.                                                                                                                       |
