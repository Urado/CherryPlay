...
3. **Freeze и завершение сессии (веб-клиент)**:
   - **Обрыв организатора** (OnConnectionStatusChanged(partyId, false)): состояние воспроизведения не сбрасывается; запускается таймер 1 мин; по истечении — скрывается блок «сейчас играет» (playbackState = null), плейлист и проигранные треки остаются. При реконнекте организатора и приходе OnSessionStarted / OnFullStateUpdated / requestFullState таймер сбрасывается.
