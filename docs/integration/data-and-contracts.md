...
- **Вечеринка**: название, описание, организатор, место, город, дата/расписание (включая время **начала** и опциональное время **окончания** мероприятия), тема, флаг «в каталоге», shortCode; для карточки каталога — краткое описание (макс. 200 символов), внешняя ссылка (URL + текст), теги танцев (до 20).
...
Поле конца мероприятия хранится в БД как `EventEndDateTime` и пробрасывается в публичные и организаторские DTO как опциональное `eventEndDateTime` (см. [CONTRACTS.md](../../CONTRACTS.md) §6.4, [CherryPlayServer/DATABASE.md](../../CherryPlayServer/DATABASE.md)). Отсутствие значения конца считается нормальным сценарием и обратно совместимо с существующими клиентами.
