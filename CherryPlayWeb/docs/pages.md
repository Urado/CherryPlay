...
- Отображение даты/времени на карточке делится на две логические строки:
  - **верхняя строка даты** остаётся прежней и показывает календарную дату события;
  - **нижняя строка интервала времени** строится по полям `eventDateTime`/`eventEndDateTime` и `timeZone` из `PublicPartyListItemDto`:
    - если заданы и начало, и конец — выводится текст **«с HH:MM до HH:MM»**;
    - если задано только начало — выводится текст **«с HH:MM»**;
    - если начало не задано (`eventDateTime` отсутствует) — нижняя строка интервала **не отображается**.
