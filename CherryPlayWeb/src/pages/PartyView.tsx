...
export const PartyView: React.FC<PartyViewProps> = ({
  ...
  // Функция для поиска длительности трека в плейлисте
  const findTrackDuration = useCallback((items: any[], id: string): number | null => {
    ...
    try {
      ...
      if (state) {
        ...
        if (state.playbackState) {
          ...
          const currentPosition = playbackStateRef.current?.position ?? state.playbackState.position;
          ...
          const finalPosition = currentLastUpdated > newLastUpdated 
            ? currentPosition 
            : state.playbackState.position;
  ...
  const signalR = useSignalR({
    ...
    onSessionStarted: useCallback((_partyId: string) => {
      setIsSessionActive(true);
      requestFullState();
    }, [setIsSessionActive, requestFullState]),
    onSessionEnded: useCallback((_partyId: string) => {
      setIsSessionActive(false);
      setPlaybackState(null);
    }, [setIsSessionActive, setPlaybackState]),
    onPlaybackPositionUpdated: useCallback((_partyId: string, trackId: string, position: number) => {
      ...
      const trackDuration = playlistRef.current ? findTrackDuration(playlistRef.current.items, trackId) : null;
    ...
    }, [findTrackDuration, setPlaybackState]),
    onFullStateUpdated: useCallback((_partyId: string, state: PlaybackStateDto) => {
      ...
      if ((!playbackState.duration || playbackState.duration === 0) && playbackState.currentTrackId && playlistRef.current) {
        const trackDuration = findTrackDuration(playlistRef.current.items, playbackState.currentTrackId);
    ...
    }, [findTrackDuration, setPlaybackState, setIsSessionActive]),
    onStateChanged: useCallback((_partyId: string) => {
      requestFullState();
    }, [requestFullState]),
    onPlaylistChanged: useCallback((_partyId: string) => {
  ...
  }, [shortCode, isDemo, loading]); // Убрали функции из зависимостей
  ...
  const displayData: PartyDisplayData = useMemo(() => {
    ...
    return data;
  }, [partyId, partyName, themeId, customizationSettings, playlist, playbackState, isSessionActive, isDemo]);
    ...
    <div className="party-view">
      <div className="party-view-container">
        <div className="party-view-header">
          ...
          {isDemo && (
            <div className="party-view-demo-badge">Демо режим</div>
          )}
        </div>
        <div className="party-view-content">
          <PartyDisplay 
            data={displayData} 
            showPlayer={!isDemo && shortCode !== undefined}
          />
        </div>
      </div>
    </div>
